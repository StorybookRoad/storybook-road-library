...
exports.create = function (student, theme_name, difficulty, done) {
	generate_story(student, theme_name, difficulty, function (err, story) { 
		...
		db.save(story, function (err, result) { 
			done(err, result);
		});
	});
};
...
function generate_story(student, theme_name, difficulty, done) {
	...
	var story = {
		...
		statistics: null //to store statistics such as # of wrong answers, etc.
	};
	
	//get the theme to populate the story object
	theme.get(theme, function (err, theme_obj) {
		if (err) return done(err, undefined);
		//get beginning phrase
		story.phrases.push(chooose_list_items(1, theme.phrases.beginning));
		//get middle phrases
		var middles = choose_list_items(STORY_LENGTH, theme.phrases.middle);
		middles.forEach(function (phrase, i) { story.phrases.push(phrase) });
		//get end phrase
		story.phrases.push(choose_list_items(1, theme.phrases.end));

		//get character and supporting
		story.words.character = choose_list_items(1, themes.characters);
		story.words.supporting = choose_list_items(1, themes.supporting);

		//get places
		theme.places.forEach(function (place, i) { story.words.places.push(place) });

		//get adjectives
		theme.adjectives.forEach(function (adj, i) { story.words.adjectives.push(adj) });

		//get answers at the correct difficulty level
		theme.answers[difficulty_str].forEach(function (adj, i) { story.words.answers.push(answer) });
		...
		story.phrases.forEach(function (phrase, i) {
			var stat_key = "question_" + i;
			var placeholder_text = "Placeholder statistics for question " + i;
...
function choose_list_items(amount, list) {
	if (list.length < 2) throw "NOT_A_LIST";
	var chosenIndicies = [];
	var chosenItems = [];
	for (var i = 0; i < amount; i++) {
		var index = Math.floor((Math.random() * list.length) +1); //get a random index
		while (chosentIndicies.indexOf(index) != -1) index = Math.floor((Math.random() * list.length) + 1); //make sure the index hasn't already been chosen
		chosenIndicies.push(index);
		chosenItems.push(list[index]);
	}
	if (amount == 1)
